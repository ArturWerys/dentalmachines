...
export default function Footer() {
  return (
    <Box component="footer" sx={{ mt: 8, py: 5, borderTop: "1px solid rgba(255,255,255,0.18)" }}>
      <Container maxWidth="lg" sx={{ px: { xs: 2, md: 3 } }}>
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "space-between" }}>
          <Box>
            ...
            <Typography sx={{ mt: 1, color: colors.textSoft, fontSize: 14, maxWidth: 360 }}>
              Lupy stomatologiczne dla ergonomii i precyzji. Dobór pod realną pracę w gabinecie.
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Typography sx={{ mt: 1.5, color: colors.textSoft, fontSize: 14 }}>
              <Link href={`mailto:${contact.email}`} underline="hover" sx={{ color: colors.textSoft }}>
                {contact.email}
              </Link>
            </Typography>
        ...
        <Box sx={{ mt: 4, pt: 3, borderTop: "1px solid rgba(255,255,255,0.18)" }}>
...
function FooterLink({ href, children }) {
  return (
    <Link
      ...
      href={href}
      underline="none"
      sx={{ color: "rgba(255,255,255,0.78)", fontSize: 14, "&:hover": { color: "white" } }}
    >
      {children}
    </Link>
  );
